...
// void foo(string N, int popsize) {
//   POPSIZE = popsize;
//   int number_of_flow = 5;

//   thread th1(Run, N, 1, "1_potok.txt", number_of_flow);
//   thread th2(Run, N, 2, "2_potok.txt", number_of_flow);
//   thread th3(Run, N, 3, "3_potok.txt", number_of_flow);
//   thread th4(Run, N, 4, "4_potok.txt", number_of_flow);
//   thread th5(Run, N, 5, "5_potok.txt", number_of_flow);
//   th1.join();
//   th2.join();
//   th3.join();
//   th4.join();
...
//  The master process initializes
  int master = 0;
  vector<string> name_file_for_potok;
  vector<string> out_name_file_for_potok;
  if ( id == 0 ) 
  {
    cout << "\n";
    cout << "GA - Master process:\n";
  ...
  for (int i =0; i<num_procs; i++){
      string name_file = to_string(i) + "_potok.txt";
      name_file_for_potok.push_back(name_file);
  }

  for (int i = 0; i<name_file_for_potok.size(); i++ ){
    cout << name_file_for_potok[i] << endl;
  }
  cout << "tut" << endl;
  // MPI_Bcast ( nums[0].c_str(), nums[0].size(), MPI_CHAR, master, MPI_COMM_WORLD );
  for (int i = 1; i<num_procs; i++ ){
    int size_name_file = name_file_for_potok[i-1].size();
    MPI_Send(&size_name_file, 1 , MPI_INT, i, 1, MPI_COMM_WORLD );
    MPI_Send( name_file_for_potok[i-1].c_str(), name_file_for_potok[i].size(), MPI_CHAR, i, 1, MPI_COMM_WORLD );
  }
  // cout << "sent" << endl;
  }else{
    int size_filename;
    MPI_Recv( &size_filename, 1, MPI_INT, master, 1, MPI_COMM_WORLD, &status );
    char name_file[size_filename+1];
    name_file[size_filename] = 0;
    MPI_Recv( &name_file, size_filename, MPI_CHAR, master, 1, MPI_COMM_WORLD, &status );
    cout << "for test id " << id << " t " << name_file << endl;
    Run(nums[0],id, name_file, num_procs);
  }
...
//                   << "popsize: " << popsize
//                   << " num: " << num
//                   << " time: " << t*1.0/CLOCKS_PER_SEC
//                   << " generations_count: " << finish_generation
//                   << " failed:" << (finish_generation == MAXGENS)
//                   << std::endl; 
//       }
//     }
//   }
